...
static HWND createEditorWindow(const WCHAR *title, BigWindow *bigWin_) {
  WNDCLASS wc = {0};

  wc.lpfnWndProc = editorWindowProc;
  wc.hInstance = GetModuleHandle(nullptr);
  wc.lpszClassName = L"eidtorWindow";
  ...
  SetWindowLongPtr(rethwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(bigWin_));
  ::ShowWindow(rethwnd, SW_SHOW);
...
bool PreloadGlobalFonts() {
  ...
  for (auto &font : g_fonts) {
    ...
    // 从文件加载字体到全局Atlas
    font.font = g_globalFontAtlas->AddFontFromFileTTF(font.filePath, font.size,
                                                      &config, ranges);
...
HWND CreateWindowWithPrivateData(const WCHAR *className, const WCHAR *title,
                                 int x, int y, int width, int height,
                                 ImGuiContext **outImguiCtx,
                                 WindowPrivateData **outWndData);
void CleanupWindowResources(HWND hWnd, ImGuiContext *ctx,
...
bool CreateDeviceD3D() {
  ...
  if (g_pd3dDevice->QueryInterface(__uuidof(IDXGIDevice),
                                   (void **)&dxgiDevice) != S_OK) {
    return false;
  }

  IDXGIAdapter *dxgiAdapter = nullptr;
  if (dxgiDevice->GetParent(__uuidof(IDXGIAdapter), (void **)&dxgiAdapter) !=
      S_OK) {
    dxgiDevice->Release();
    return false;
  }

  if (dxgiAdapter->GetParent(__uuidof(IDXGIFactory), (void **)&g_pFactory) !=
      S_OK) {
    dxgiAdapter->Release();
    dxgiDevice->Release();
    return false;
...
// 共享窗口回调函数
LRESULT WINAPI SharedWndProc(HWND hWnd, UINT msg, WPARAM wParam,
                             LPARAM lParam) {
  WindowPrivateData *pWndData = reinterpret_cast<WindowPrivateData *>(
  ...
  switch (msg) {
  case WM_SIZE:
    if (pWndData != nullptr && g_pd3dDevice != nullptr &&
      ...
      pWndData->swapChain->ResizeBuffers(0, newWidth, newHeight,
                                         DXGI_FORMAT_UNKNOWN, 0);

      ID3D11Texture2D *pBackBuffer = nullptr;
      if (pWndData->swapChain->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer)) ==
          S_OK) {
...
HWND CreateWindowWithPrivateData(const WCHAR *className, const WCHAR *title,
                                 ...
                                 ImGuiContext **outImguiCtx,
                                 WindowPrivateData **outWndData) {
  HWND hWnd = CreateWindowExW(WS_EX_LAYERED, className, title, WS_POPUP, x, y,
  ...
  *outWndData = new WindowPrivateData();
  ...
  ID3D11Texture2D *pBackBuffer = nullptr;
  if ((*outWndData)->swapChain->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer)) ==
      S_OK) {
    g_pd3dDevice->CreateRenderTargetView(pBackBuffer, nullptr,
                                         &(*outWndData)->rtv);
    pBackBuffer->Release();
