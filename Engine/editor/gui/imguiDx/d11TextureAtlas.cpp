...
TextureAtlas CreateTextureAtlas(ID3D11Device *device) {
  ...
  for (const char *path : imagePaths) {
    int w, h, channels;
    stbi_uc *pixels =
        stbi_load(path, &w, &h, &channels, STBI_rgb_alpha); // 统一转为RGBA
    if (!pixels) {
      // 错误处理：释放已加载的资源并返回空纹理集
      for (auto p : subimagePixels)
        stbi_image_free(p);
      return atlas;
    }
    subimagePixels.push_back(pixels);
    tempSubimages.push_back(
        {path, 0, 0, w, h, 0, 0, 0, 0}); // 暂存尺寸，坐标后续计算
  }
  ...
  for (const auto &sub : tempSubimages) {
    maxSubimageHeight =
        (((maxSubimageHeight) > (sub.height)) ? (maxSubimageHeight)
                                              : (sub.height));
  ...
  for (size_t i = 0; i < tempSubimages.size(); ++i) {
    ...
    for (int y = 0; y < sub.height; ++y) {
      // 计算纹理集当前行的起始地址
      stbi_uc *atlasRow = atlasPixels +
                          (sub.y + y) * atlas.atlasWidth * pixelSize +
                          sub.x * pixelSize;
      // 计算子图像当前行的起始地址
